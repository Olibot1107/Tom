...
Includes software simulation mode for development without hardware
"""
...
try:
    ...
    print("Warning: luma.oled not installed. Running in software simulation mode.")
...
class DisplayType(Enum):
    ...
    SOFTWARE = "software"  # Software simulation mode (no hardware needed)
...
class MockDevice:
    """Mock device for software simulation mode (no hardware required)"""

    def __init__(self, width=128, height=64, mode='1'):
        self.width = width
        self.height = height
        self.mode = mode
        self._buffer = Image.new(mode, (width, height))
        self._contrast = 255
        self._hidden = False

    def display(self, image):
        """Store the image but don't output to hardware"""
        if not self._hidden:
            self._buffer = image.copy()
            # In simulation mode, we could save to file or display in window
...
class PiOLED:
    ...
    Works with or without physical I2C/SPI hardware
    """
    ...
    def __init__(
        ...
        height: int = None,
        rotate: int = 0,
        simulation_mode: bool = False  # Force software mode even if luma is available
    ):
        """
        ...
        Args:
            ...
            simulation_mode: Force software simulation (no hardware needed)
        """
        self.display_type = display_type
        self.interface = interface
        self.simulation_mode = simulation_mode or not LUMA_AVAILABLE

        # Determine if color display
        if display_type == DisplayType.SOFTWARE:
            self.is_color = False  # Default to monochrome for software mode
        else:
            self.is_color = (display_type == DisplayType.SSD1331)

        # Set default dimensions based on display type
        if width is None:
            if display_type == DisplayType.SSD1331:
                width = 96
            else:
                width = 128
        ...
        # Initialize device
        if self.simulation_mode or display_type == DisplayType.SOFTWARE:
            self.device = MockDevice(width=self.width, height=self.height, 
                                    mode='RGB' if self.is_color else '1')
            print(f"Software simulation mode: {self.width}x{self.height} display")
        else:
            try:
    ...
    def show(self):
        ...
        # In simulation mode, optionally save to file for viewing
        if self.simulation_mode:
            try:
                # Convert to viewable format and save
                if self.mode == '1':
                    display_buffer = self.buffer.convert('RGB')
                else:
                    display_buffer = self.buffer
                # Uncomment to save frames:
                # display_buffer.save(f"simulation_frame_{int(time.time()*1000)}.png")
            except:
                pass
    ...
    def screenshot(self, filename: str = "screenshot.png"):
        ...
        if self.mode == '1':
            # Convert monochrome to RGB for better viewing
            rgb_buffer = self.buffer.convert('RGB')
            rgb_buffer.save(filename)
        else:
            self.buffer.save(filename)
        print(f"Screenshot saved to {filename}")

    def save_frame(self, filename: str = None):
        """Save current frame (useful for simulation mode)"""
        if filename is None:
...
def create_software(width=128, height=64, color=False, **kwargs):
    """
    Create software simulation display (no hardware required)

    Args:
        width: Display width
        height: Display height  
        color: True for RGB color mode, False for monochrome
    """
    display = PiOLED(
        display_type=DisplayType.SOFTWARE,
        width=width,
        height=height,
        simulation_mode=True,
        **kwargs
    )
    display.is_color = color
    display.mode = 'RGB' if color else '1'
    display.buffer = Image.new(display.mode, (width, height))
    display.draw = ImageDraw.Draw(display.buffer)
    return display
...
if __name__ == "__main__":
    ...
    # Create display - automatically uses simulation mode if no hardware
    # Option 1: Try hardware first, fallback to software:
    # display = create_ssd1306()

    # Option 2: Force software mode (no I2C/SPI hardware needed):
    display = create_software(width=128, height=64, color=False)

    # Option 3: Software color display:
    # display = create_software(width=96, height=64, color=True)
    ...
    try:
        ...
        display.show()
        display.save_frame("demo1_shapes.png")
        time.sleep(2)
        ...
        display.show()
        display.save_frame("demo2_text.png")
        time.sleep(2)

        # Demo 3: Loading animations
        print("Demo 3: Loading Animations (3 seconds each)")
        ...
        for style in LoadingStyle:
            ...
            display.save_frame(f"demo3_{style.value}.png")
        ...
        for i in range(0, 101, 10):
            ...
            display.show()
            display.save_frame(f"demo4_progress_{i}.png")
            time.sleep(0.1)
        ...
        display.scroll_text("This is a scrolling text demo! ", 
                          y=20, speed=0.02)
        display.save_frame("demo5_scroll.png")
        ...
        for img in test_images:
            if os.path.exists(img):
                ...
                display.image(img)
                display.show()
                display.save_frame(f"demo6_{img}.png")
                time.sleep(2)
        ...
        print("Demo complete!")
        print(f"Frames saved as demo*.png files")
